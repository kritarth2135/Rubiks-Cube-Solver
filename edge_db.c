...
int create_edge_position_db(
    ...
    if (*depth < max_depth) {
        ...
        for (int i = 0; i < NUMBER_OF_BASIC_MOVES; i++) {
            ...
            make_move(cube, BASIC_MOVES[i]);
            uint64_t index = encode_all_edge_positions(cube);
            if (index >= POSSIBLE_EDGE_POSISION_COMBINATIONS) {
                return 1;
            }
            (*no_of_nodes_processed)++;
            ...
            set_four_bits(edge_db, index, *depth);
            if (create_edge_position_db(edge_db, cube, depth, max_depth, no_of_nodes_processed, prev_moves_indexes) == 1) {
                return 1;
            }
            make_move(cube, REVERSE_BASIC_MOVES[i]);
        }
...
uint8_t * load_edge_position_db() {
    uint8_t *db = load_db(EDGE_POSITIONS_DB_NAME, EDGE_DB_GENERATION_MAX_DEPTH, POSSIBLE_EDGE_POSISION_COMBINATIONS, create_edge_position_db);
    if (db == NULL) {
        return NULL;
    }
    return db;
